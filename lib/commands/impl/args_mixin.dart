...
mixin ArgsMixin {
  final List<String> _args = VortexCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(VortexCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return VortexCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < VortexCli.arguments.length) {
      index++;
      return VortexCli.arguments[index];
    } else {
